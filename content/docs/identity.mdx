...
3. Look up the row by `pat_id`. If no row matches, run a dummy Argon2id verification against a fixed hash to equalize wall-clock time, then raise `InvalidPatTokenError` (conflated shape — no timing oracle on token presence).
