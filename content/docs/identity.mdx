...
Recovery codes are generated as a set and stored hashed. Each code is single-use; the store marks it consumed on verify. When a user runs out of codes, issue a new set via `enrollMfaFactor(usr_id, type="recovery")`.
...
1. Parse the token into `(id_part, secret_part)` at the last `_` before the base64url run.
2. Look up the row by `pat_id`. If no row matches, raise `InvalidPatTokenError`.
3. Argon2id-verify `secret_part` against the stored `secret_hash`. If mismatch, raise `InvalidPatTokenError` (conflated shape — no timing oracle on token presence).
4. If `revoked_at` is non-null, raise `InvalidPatTokenError`.
5. If `expires_at` is set and `expires_at <= now`, raise `InvalidPatTokenError`.
6. Set `last_used_at = now` (best-effort; non-transactional).
7. Return the verified `usr_id`.
...
| `system` | Internal / unauthenticated system action |
...
18 RFC 6238 §B test vectors are pinned in `spec/conformance/identity/mfa/totp-rfc6238.json`. Every SDK MUST pass all 18.

### MFA recovery-code format (v0.2)

Recovery codes MUST be 31 characters from the allowed alphabet (excluding `0/O/1/I/L`). 12 fixture tests in `spec/conformance/identity/mfa/recovery-code-format.json`.

### WebAuthn assertion (v0.2 + v0.3)

7 ES256 + 6 algorithm-dispatch (RS256, EdDSA) tests in `spec/conformance/identity/mfa/webauthn-assertion.json` and `webauthn-assertion-algorithms.json`. Counter-decrease rejection: 4 tests.

### PAT token format (v0.3)

11 structural-validation tests in `spec/conformance/identity/pat/token-format.json` pinning the `pat_<32hex>_<base64url>` wire-format rules.
